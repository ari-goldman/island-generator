...
unsigned int LoadTexBMP(const char* file)
{
   ...
   // float* imageRGBA = (float*) malloc(sizeRGBA * sizeof(float));
   std::vector<float> imageRGBA;


   // Convert 8-bit RGB data to floating-point RGBA
   for(unsigned int i = 0; i < dx * dy; i++) {
      imageRGBA.push_back(image[i * 3]     / 255.0f);     // R
      imageRGBA.push_back(image[i * 3 + 1] / 255.0f);     // G
      imageRGBA.push_back(image[i * 3 + 2] / 255.0f);     // B
      imageRGBA.push_back(1.0f);                          // A (fully opaque)
   }
   ...
   //  Copy image
   glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, dx, dy, 0, GL_RGBA, GL_FLOAT, imageRGBA.data());
   ...
   //    imageRGBA[i*4] = image[i*3];
   //    imageRGBA[i*4 + 1] = image[i*3 + 1];
   //    imageRGBA[i*4 + 2] = image[i*3 + 2];
   //    imageRGBA[i*4 + 3] = 255;
   // }
   ...
   // glBindTexture(GL_TEXTURE_2D,texture);
   // //  Copy image
   // glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA8,dx,dy,0,GL_RGBA,GL_UNSIGNED_BYTE,imageRGBA);
   // if (glGetError()) Fatal("Error in glTexImage2D %s %dx%d\n",file,dx,dy);
   // //  Scale linearly when image size doesn't match
   // glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
   // glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
